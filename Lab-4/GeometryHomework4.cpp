// Lab 4B, Geometrey Homework Program
...
public:
    Cube(const char* const token[4]):cube_side(token[1] == 0?0:atof(token[1]))
    {
...
class Prism {
private:
    const double prism_length;
    const double prism_width;
    const double prism_height;
public:
    Prism(const char* const token[4]):prism_length(token[1] == 0?0:atof(token[1])), prism_width(token[2] == 0?0:atof(token[2])), prism_height(token[3] == 0?0:atof(token[3]))
    {
...
public:
    Cylinder(const char* const token[4]):cylinder_radius(token[1] == 0?0:atof(token[1])), cylinder_height(token[2] == 0?0:atof(token[2]))
    {
